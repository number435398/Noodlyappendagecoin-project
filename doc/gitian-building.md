...
*Setup instructions for a Gitian build of Noodlyappendagecoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Noodlyappendagecoin
...
- [Building Noodlyappendagecoin Core](#building-noodlyappendagecoin-core)
...
Clone the git repositories for noodlyappendagecoin and Gitian.
...
in the noodlyappendagecoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Noodlyappendagecoin Core
----------------

To build Noodlyappendagecoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the noodlyappendagecoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/noodlyappendagecoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/noodlyappendagecoin-project/noodlyappendagecoin
...
For example:
```bash
URL=https://github.com/thrasher-/noodlyappendagecoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit noodlyappendagecoin=${COMMIT} --url noodlyappendagecoin=${URL} ../noodlyappendagecoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit noodlyappendagecoin=${COMMIT} --url noodlyappendagecoin=${URL} ../noodlyappendagecoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit noodlyappendagecoin=${COMMIT} --url noodlyappendagecoin=${URL} ../noodlyappendagecoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the noodlyappendagecoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../noodlyappendagecoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/noodlyappendagecoin
SIGPATH=/some/root/path/noodlyappendagecoin-detached-sigs

./bin/gbuild --url noodlyappendagecoin=${BTCPATH},signature=${SIGPATH} ../noodlyappendagecoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/noodlyappendagecoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/noodlyappendagecoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/noodlyappendagecoin-osx-build.assert
```
...
[noodlyappendagecoin-project/gitian.sigs.ltc](https://github.com/noodlyappendagecoin-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
